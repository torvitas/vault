...
function build() {
  ...
  if [ ${GO_TOOLCHAIN} == "fips" ]; then
    mkdir .tools
    cd .tools
    curl --show-error --silent --fail --user "${ARTIFACTORY_USER}:${ARTIFACTORY_TOKEN}" --request GET \
    "${ARTIFACTORY_ENDPOINT}/${ARTIFACTORY_SOURCE_REPO}/fips-golang-go/${FIPS_GO_VERSION}/${FIPS_GO_COMMIT}/${FIPS_GO_ARTIFACT}" \
    --output go.tar.gz
    echo "$FIPS_GO_CHECKSUM  go.tar.gz" | sha256sum -c -
    sudo apt update && sudo apt install -y clang
    mkdir go && tar --strip-components=2 -C go -xf go.tar.gz
